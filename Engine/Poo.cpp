...
poo::poo(int X, int Y, int vx, int vy, dude &d)
{
...
void poo::colloide_boundaries(Graphics &gfx)
{
	if (x <= 0)
	{
		x = 0;
...
void poo::calculate_pos(Graphics &gfx)
{
	colloide_boundaries(gfx);
	x += dx;
	y += dy;

}

void poo:: GotEaten(dude &dude)
{  
	int dude_right = dude.GetX() + dude.GetWidth();
	int dude_bottom = dude.GetY() + dude.getheight();
	int poo_right = x + width;
	int poo_bottom = y + height;
	if ((dude_right >= x) && (poo_right >= dude.GetX()) && (dude.GetY() <= poo_bottom) && (y <= dude_bottom))
		isEaten = true;
}
bool poo::IsEaten()
{
